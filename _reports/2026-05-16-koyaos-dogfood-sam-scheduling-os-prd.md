...
revising it from the operator's own answers surfaced eight more (REQ-19 through
REQ-26). REQ-19–23 came from writing the PRD; REQ-24–25 came from updating it
once Samantha answered the Group A questions; REQ-26 came from a follow-up
operator correction that tightened the build wedge. Each is grounded in something
the PRD work actually hit.
...
## REQ-26 — Operator discovery must separate the job-to-be-done from adjacent complexity

**What happened:** The PRD was built around a "guided scheduler" — a
scheduling-rules model, capacity math, large-dog-run limits, crate-capacity
warnings, a duration estimator with per-dog overrides. That is the
intellectually interesting part of Samantha's workflow, and the kickoff name
("Scheduling OS") pointed straight at it. Then Russell relayed a one-line
operator correction: the first product win is not scheduling automation, it is
**fast lookup when a client calls or texts.** Samantha said scheduling
*judgment* is not her hard problem — she is good at deciding what fits a day. Her
actual pain is mundane: pulling the right household and its history up fast
enough to book on the call. The whole milestone plan re-ordered — the
lookup-and-book wedge moved to M3, ahead of the scheduling intelligence that had
been the PRD's centre of gravity.

The complexity the PRD had carefully modelled was real — but it was *adjacent* to
the job to be done, not the job itself. The agent, and the kickoff, had
over-indexed on the sophisticated part of the workflow and under-weighted the
high-frequency, low-glamour part.

**What Koya needs:** When an agent captures an operator's workflow, Koya should
force an explicit separation between **the job to be done** — the single most
frequent, most painful task the operator repeats — and **adjacent complexity** —
the parts of the domain that are intricate and modelable but not where the pain
is. Sophistication attracts planning attention; frequency and pain should set
priority. Koya's operator-knowledge planning template (REQ-20) should add a
required field per workflow area: *how often does the operator do this, and how
much does it hurt?* — and rank build order by frequency × pain, not by how
interesting the domain model is. A feature plan whose first milestone is the most
elaborate subsystem rather than the most repeated interaction should trip a
check. This pairs with REQ-24 (ask the operator) and REQ-25 (elicit the
operator's language): the same direct-question discipline that resolved the
graduation gate and the editing-workflow framing also surfaces which task is
actually the wedge.
...
| REQ-26 | Separate the job-to-be-done from adjacent complexity | P1 | Koya platform / planning |

---

*Generated by CC 2026-05-16. Derived from producing the Tidy Tails v2 Sam
Scheduling OS PRD (`_reports/2026-05-16-sam-scheduling-os-prd.md`), revising it
from Samantha's Group A answers, and then revising it again from a follow-up
operator correction that tightened the build wedge. Continues
